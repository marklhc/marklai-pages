...
each level, number of participants per country, etc), as well as the descriptive statistics for the three variables `gm_GNP`, `income`, and `happy`. Indicate at which level each of the three variables are defined.
3. Fit a random intercept model (i.e., `Happy` as the outcome variable and no
predictor in the model)
    i. Calculate the intraclass correlation
    ii. Adapt the [code in the "Showing the variations" section](../rcode3/#showing-the-variations-across-a-subset-of-schools) to plot the distribution of `Happy` across a random sample of 5-10 countries. (Hint: you should use the `country` variable for plotting as that has the names of the countries)
