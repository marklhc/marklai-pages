...
\begin{align*}
  ...
  \mu_j & \sim N(\beta_0 + \beta_2 X_{.j}, \tau) \\
  \beta & \sim N(0, \inf) \\
  \sigma & \sim t_3(0, 10) \\
  \tau & \sim t_3(0, 10)
\end{align*}
...
\begin{align*}
  ...
  X_{ij} & \sim N(\mu_{Xj}, \sigma^2_X) \\
  \mu_j & \sim N(0, \tau_X) \\
  \mu_{Xj} & \sim N(\beta_0 + \beta_2 \mu_{Xj}, \tau) \\
  \sigma & \sim t_3(0, 10) \\
  \tau & \sim t_3(0, 10) \\
...
```{r}
print(m2_stan, pars = c("b0", "b", "bw", "b_contextual", "sigma_y", "tau_y", 
                        "sigma_x", "tau_x"))
```

```{r, include=FALSE}
bw <- as.matrix(m2_stan, "bw")
bcon <- as.matrix(m2_stan, "b_contextual")
```

Note that the coefficient of `SES` at level-2 now has a posterior mean of 
`r comma(mean(bw))` with a posterior *SD* of 
`r comma(sd(bw))`, and the contextual effect has a posterior mean
...
```{r}
print(m2full_stan, pars = c("b0", "b", "bw", "b_contextual", 
