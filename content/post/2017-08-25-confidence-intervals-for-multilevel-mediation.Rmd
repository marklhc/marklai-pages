...
The two main analytical approaches are:
...
And we replace the $a$, $b$, $\SE(a)$, $\SE(b)$ with the corresponding 
sample estimates. Note that as we use separate equations to obtain $\hat a$ and 
...
system.time(med_boo <- boot(school_sub, ab_resample, 1999))
med_boo
```
...
`r comma((mean(med_boo$t[ , 2]) / med_boo$t0["vab"] - 1) * 100)`%!

### Bootstrap CI

```{r}
# For (unstandardized) indirect effect
boot.ci(med_boo, type = "all", index = 1:2)
```

Generally BCa (bias-corrected and accelerated) and studentized CIs are more 
accurate (see Davison & Hinkley, 1997; Van der Leeden, Meijer, & Busing, 2008). 
However, the BCa needs the accerlation value (which is used to correct for 
skewness), which is estimated using the regression method here, and to my 
knowledge its use for multilevel data has not been tested. 
...
the indirect effect in each bootstrap replication. However, for the 
$y$-standardized indirect effect and the proportion mediated it would be 
more work to obtain the sampling variance estimates (which can be obtained 
with the delta method or with other resampling method within each bootstrap
sample; the former needs some derivation and may not be very accurate and the
latter will increase the computation time by a factor of 100 or more, which is 
not practical), so I did not request for the studentized CI. 
...
We also can get the bootstrap CIs for the indirect effect with $y$ 
standardized, referred to as $ab_{ps}$ in the literature, 
which has the interpretation of:

> Every unit increase in $x$ (treatment variable) results in $ab_{ps}$ 
> standard deviations in $y$ through the mediator. 
...
confidence limits. For single-level mediation Cheung (2009) showed that the 
...
good measure of effect size, as also pointed out in Miocevic, O'Rourke,
MacKinnon, and Brown (2017).
