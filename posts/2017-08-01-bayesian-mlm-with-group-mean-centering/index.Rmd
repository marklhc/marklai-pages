...
# Transform data
hsb_mdv <- hsb_sub %>%
  # duplicate original predictor
  mutate(pred_SES = SES) %>%
  pivot_longer(cols = c(MATHACH, SES), names_to = "var")
brm(bf(value ~ b1 * pred_SES + b2 * partx + b3 * party,
       b1 ~ 0 + var,
       b2 ~ 0 + var,
       b3 ~ 0 + var,
       partx ~ (1 | ID),
    ...
    data = hsb_mdv,
    prior = c(
      prior(student_t(4, 0, 10), coef = "varMATHACH", nlpar = "b1"),
      prior(constant(0), coef = "varSES", nlpar = "b1"),
      prior(student_t(4, 0, 10), coef = "varMATHACH", nlpar = "b2"),
      prior(constant(1), coef = "varSES", nlpar = "b2"),
      prior(constant(1), coef = "varMATHACH", nlpar = "b3"),
      prior(constant(0), coef = "varSES", nlpar = "b3")
    ),
    iter = 2000,
    chains = 2)
# Compare
summary(lmer(SES ~ (1 | ID), data = hsb_sub))
summary(lmer(MATHACH ~ MEANSES + SES + (1 | ID), data = hsb_sub))
```
